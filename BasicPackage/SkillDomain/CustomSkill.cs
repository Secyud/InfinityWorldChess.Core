...
namespace InfinityWorldChess.SkillDomain
{
    public class CustomSkill : IActiveSkill, IArchivableShown, IArchivable
    {
        ...
        public void SetContent(Transform transform)
        {
            ...
            {
                if (Condition is IHasContent content)
            ...
            {
                if (Result is IHasContent content)
                    content.SetContent(transform);
            }
            {
                if (Condition is IHasContent content)
        ...
        {
            foreach (ISkillCastEffect effect in Effects)
                if (effect is IReleasable releasable)
                    releasable.Release();
            {
        ...
        public void Cast(BattleRole role, HexCell releasePosition)
        {
            foreach (ISkillCastEffect effect in Effects)
                effect.Cast(role, releasePosition);
        }
