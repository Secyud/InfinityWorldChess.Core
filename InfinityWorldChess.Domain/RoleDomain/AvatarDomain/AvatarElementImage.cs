...
namespace InfinityWorldChess.RoleDomain
{
    public class AvatarElementImage : SImage
    {
        ...
        {
            if (_container is null ||
                !_container.Message.MovableX) return;
            float value =
                Mathf.Lerp(
        ...
        {
            if (_container is null ||
                !_container.Message.MovableY) return;
            float value =
                Mathf.Lerp(
                    -_container.Message.YRange,
                    _container.Message.YRange,
                    GetRange01(positionY)) +
        ...
        {
            if (_container is null ||
                !_container.Message.Rotatable) return;
            float value = Mathf.Lerp(
