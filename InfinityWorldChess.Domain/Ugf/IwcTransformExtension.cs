...
namespace InfinityWorldChess.Ugf
{
	public static class IwcTransformExtension
	{
		...
		public static void AddEquipmentProperty(this Transform transform, IEquipment equipment)
		{
			...
			string text = "位置:".Point();
			if (equipment.CanSet(0)) text += " 生";
			if (equipment.CanSet(1)) text += " 杀";
			if (equipment.CanSet(2)) text += " 灵";
			if (equipment.CanSet(3)) text += " 御";
			transform.AddParagraph(text);

			text = "基础:".Point();
			text += (equipment.TypeCode & 0b1) > 0 ? " 短基" : " 长基";
			text += (equipment.TypeCode & 0b10) > 0 ? " 一般" : " 加长";
			transform.AddParagraph(text);

			text = "刃型".Point();
			text += (equipment.TypeCode & 0b10) > 0 ? " 有" : " 无";
			text += (equipment.TypeCode & 0b1000) > 0 ? " 轻" : " 重";
			text += (equipment.TypeCode & 0b10000) > 0 ? " 软" : " 硬";
			text += (equipment.TypeCode & 0b100000) > 0 ? " 双" : " 单";
			transform.AddParagraph(text);

			text = "类型".Point();
			text += (equipment.TypeCode & 0b1000000) > 0 ? " 远程" : " 近战";
			text += (equipment.TypeCode & 0b10000000) > 0 ? " 变体" : " 普通";
			transform.AddParagraph(text);
		...
		public static void AddPassiveSkillInfo(this Transform content, IPassiveSkill passiveSkill)
		{
			...
			content.AddParagraph($"[阴]: {passiveSkill.Yin,-5} [阳]: {passiveSkill.Yang,-5}".Point());
